...
from torchnmf.nmf import NMF as torch_NMF
import torch
...
def separate(
    ...
    ############################ new code start ###############################
    elif sep_method == 'nmf_cuda':
        print("running nmf with torch and cuda")
        for ith_try in range(maxtries):
            
            # Make an instance of the torch NMF class
            if W0 is None and H0 is None:
                S = torch.from_numpy(S)
                S = S.cuda()
                nmf = torch_NMF(S.T.shape).cuda()

                # Perform NMF and find separated signals
                # tol=1e-5
                # alpha=0.1
                n_iter_ = nmf.fit(S.T, tol=tol, max_iter=maxiter, alpha=alpha, l1_ratio=0.5)
                S_sep = nmf.H
            else:
                S = torch.from_numpy(S)
                S = S.cuda()
                nmf = torch_NMF(S.T.shape, W=W0, H=H0,
                    rank=n).cuda()
        ...
        if n_iter_ == maxiter - 1:
            print((
                'Warning: maximum number of allowed tries reached at {} '
                'iterations for {} tries of different random seed states.'
            ).format(n_iter_ + 1, ith_try + 1))

        A_sep = nmf.W
        
        S_sep = S_sep.cpu().detach().numpy().T
        A_sep = A_sep.cpu().detach().numpy().T
        S = S.cpu().detach().numpy()

        # cuda mixes up the signals during nmf - likely due to the paralleliza-
        # tion process. the following unmixes based on correlation with the
        # raw signal
        new_inds = []
        for orig_roi in S:
            max_corr = 0
            corr_ind = -100
            for ind, cuda_roi in enumerate(S_sep):
                corr, _ = pearsonr(cuda_roi, orig_roi)
                if corr > max_corr:
                    corr_ind = ind
                    max_corr = corr
            new_inds.append(corr_ind)
            
        S_sep = S_sep[new_inds].T
        A_sep = A_sep[new_inds].T
    ...
    if sep_method == 'ica':
        ...
        convergence['random_state'] = random_state
        convergence['iterations'] = n_iter_
        convergence['converged'] = not n_iter_ == maxiter
